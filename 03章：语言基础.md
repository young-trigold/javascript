...
使用 Number.EPSILON 可以避免这个问题。Number.EPSILON 属性表示 1 与Number可表示的大于 1 的最小的浮点数之间的差值。EPSILON 属性的值接近于 2.2204460492503130808472633361816E-16，或者 2-52。

以下例子使用Number.EPSILON验证浮点数的相等：
...
双精度浮点数使用64个位存储。这64位从最高到低由1个符号位，11个阶码位，52个尾数位组成。
...
双精度浮点数规定，-4 + 1023 = 1019 再转为 11 位二进制数 01111111011 为阶码。

3. **尾数位**

尾数位是最有意思的部分。因为在52位的有限位内存储无限的数字就要涉及截取和舍入问题。

上面提到，十进制小数0.1转为科学计数法的二进制小数，为：

2^-4 * 1.10011001100110011001100110011001100110011001100(1100)+

IEEE 754 规定，将这个式子中的 1.10011001100110011001100110011001100110011001100(1100)+ 去掉 "1." 后再截取或舍入的52位数字称为尾数。在这个例子中，就牵扯到舍入。
...
但注意到第52位后面的数字为100。在这种情况下，IEEE 754 规定满1进位。于是这个数字舍入后变为：
...
有限的52个尾数位再加上省略的"1." 转为十进制小数科学计数法时，只有16个小数位数的精度 (2^53=9007199254740992，长度为 16位) 。

上述的0.1的位模式实际上对应的十进制小数为：

0.1000000000000000055511151231257827021181583404541015625

但保留16位后，变为：

10^-1 * 1.0000000000000000

即 0.10000000000000000

5. **0.1 + 0.2**

0.1 和 0.2 的对应的64位浮点数的科学计数法为：
...
sum =  2^-3 * 10.0110011001100110011001100110011001100110011001100111 = 2^-2  * 1.0011001100110011001100110011001100110011001100110011(1)

这里尾数部分有 53 位，舍入52位后为：
