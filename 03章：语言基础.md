...
- 如果两个操作数都不是字符串，则把操作数使用 Number() 转换为数值，执行数值比较。数值比较有以下规则：
...
- 如果任一操作符是数值，则将另一个操作数转换为数值后再进行数值比较。
...
const f1 = NaN == NaN; // false
const f2 = 0 == null; // false
const t2 = 0 == ""; // true
const t3 = 0 == []; // true
const f3 = "" == []; // false
```

比较反直觉的是：在 `0 == "0"` 中，由于 0 是数值，所以会对 "0" 执行数值转换，结果为 true。在 `0 == []` 中，[] 转换结果为 0，所以也是 true。但是在 `"" == []` 中没有数值，所以不进行数值转换，因此 "" 和 [] 不相等。
