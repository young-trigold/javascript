...
- [1.2. 创建对象](#12-创建对象)
  - [1.2.1. 工厂模式](#121-工厂模式)
  - [1.2.2. 构造函数模式](#122-构造函数模式)
  - [1.2.3. 原型模式](#123-原型模式)
  - [1.2.4. 理解原型](#124-理解原型)
- [1.3. 继承](#13-继承)
  - [1.3.1. 原型链](#131-原型链)
  - [1.3.2. 盗用构造函数](#132-盗用构造函数)
  - [1.3.3. 组合继承](#133-组合继承)
  - [1.3.4. 原型式继承](#134-原型式继承)
  - [1.3.5. 寄生式继承](#135-寄生式继承)
  - [1.3.6. 寄生式组合继承](#136-寄生式组合继承)
- [1.4. 类](#14-类)
  - [1.4.1. 类定义](#141-类定义)
  - [1.4.2. 类构造函数](#142-类构造函数)
  - [1.4.3. 实例，原型和类成员](#143-实例原型和类成员)
  - [1.4.4. 继承](#144-继承)
...
const book = (function () {
  ...
  const _book = new Object();
  _book.edition = 4;

  Object.defineProperty(_book, 'year', {
    get() {
      return _year;
    },
    set(newValue) {
...
在这个例子中，对象 book 有两个默认属性：\_year 和 edition。\_year 中的下划线常用来表示该属性私有。另一个属性 year 被定义为一个访问器属性，其中获取函数简单地返回 \_year 的值，而设置函数会做一些计算以决定正确的版本（edition）。因此，把 year 属性修改为 2022 会导致 \_year 变成 2022，edition 变成 6。这是访问器属性的典型使用场景，即设置一个属性值会导致一些其他变化发生。
...
这段代码在 book 对象上定义了两个数据属性 \_year 和 edition，还有一个访问器属性 year。最终的对象跟上一节示例中的一样。唯一的区别是所有属性都是同时定义的，并且数据属性的 configurable、enumerable 和 writable 特性值都是 false。
...
对于数据属性 \_year，value 等于原来的值，configurable 是 false，get 是 undefined。对于访问器属性 year，value 是 undefined，enumerable 是 false，get 是一个指向获取函数的指针。
