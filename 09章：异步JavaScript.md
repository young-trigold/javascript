- [1.1. 事件循环](#11-事件循环)
  - [1.1.1. 同步与异步](#111-同步与异步)
  - [1.1.2. 事件循环模型](#112-事件循环模型)
  - [1.1.3. 宏任务和微任务](#113-宏任务和微任务)
- [1.2. 期约](#12-期约)
  - [1.2.1. 期约基础](#121-期约基础)
    - [1.2.1.1. 期约状态机](#1211-期约状态机)
    - [1.2.1.2. 解决值，拒绝理由及期约用例](#1212-解决值拒绝理由及期约用例)
    - [1.2.1.3. 通过执行函数控制期约状态](#1213-通过执行函数控制期约状态)
    - [1.2.1.4. Promise.resolve()](#1214-promiseresolve)
    - [1.2.1.5. Promise.reject()](#1215-promisereject)
    - [1.2.1.6. 同步/异步执行的二元性](#1216-同步异步执行的二元性)
  - [1.2.2. 期约实例的方法](#122-期约实例的方法)
    - [1.2.2.1. 实现 Thenable 接口](#1221-实现-thenable-接口)
    - [1.2.2.2. then()](#1222-then)
    - [1.2.2.3. catch()](#1223-catch)
    - [1.2.2.4. finally()](#1224-finally)
    - [1.2.2.5. 非重入期约方法](#1225-非重入期约方法)
    - [1.2.2.6. 邻近处理程序的执行顺序](#1226-邻近处理程序的执行顺序)
    - [1.2.2.7. 传递解决值和拒绝理由](#1227-传递解决值和拒绝理由)
    - [1.2.2.8. 拒绝期约与拒绝错误处理](#1228-拒绝期约与拒绝错误处理)
  - [1.2.3. 期约连锁与期约合成](#123-期约连锁与期约合成)
    - [1.2.3.1. 期约连锁](#1231-期约连锁)
    - [1.2.3.2. 期约图](#1232-期约图)
    - [1.2.3.3. Promise.all() 与 Promise.race()](#1233-promiseall-与-promiserace)
    - [1.2.3.4. 串行期约合成](#1234-串行期约合成)
  - [1.2.4. 期约扩展](#124-期约扩展)
    - [1.2.4.1. 期约取消](#1241-期约取消)
    - [1.2.4.2. 期约进度通知](#1242-期约进度通知)
- [1.3. 异步函数](#13-异步函数)
  - [1.3.1. 异步函数](#131-异步函数)
    - [1.3.1.1. async](#1311-async)
    - [1.3.1.2. await](#1312-await)
    - [1.3.1.3. await 的限制](#1313-await-的限制)
  - [1.3.2. await](#132-await)
  - [1.3.3. 异步函数策略](#133-异步函数策略)
    - [1.3.3.1. 实现 sleep()](#1331-实现-sleep)
    - [1.3.3.2. 利用平行执行](#1332-利用平行执行)
    - [1.3.3.3. 串行执行期约](#1333-串行执行期约)
    - [1.3.3.4. 栈追踪与内存管理](#1334-栈追踪与内存管理)
...
然而还存在另外一个组件: **事件队列**。这个组件的作用我们接下来会讲到。下图展示了整体的组件架构。
...
#### 1.2.3.3. Promise.all() 与 Promise.race()
