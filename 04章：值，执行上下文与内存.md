...
JavaScript 有 3 种可执行代码：

1. **全局可执行代码**

全局可执行代码是指任何函数体的外部的脚本代码，当 JavaScript 引擎第 1 次加载脚本文件时，执行流就会进入一个 **全局执行上下文(Gobal Execution Context, GEC)**。
...
那么当该脚本被执行时，首先创建全局执行上下文 GEC，并推入 ECS，此时 GEC 变为：
...
在执行 baz 函数时，JavaScript 执行流又遇到了函数 bar 的调用，于是 JavaScript 引擎创建了 barEC，并推入 ECS：
...
当函数 foo 执行完成之后，ECS 便弹出 foo 的执行上下文：
...
如果页面没有被关闭，ECS 栈底始终有 GEC，如果页面也被关闭，那么 GEC 也被弹出执行上下文栈：
...
从这个例子可以看出，如果函数被递归调用，那么 ECS 便会不断的被推入函数执行上下文：
...
每一个执行上下文都包含了 3 个额外的状态组件：词法环境，变量环境，以及 ThisBinding。

**词法环境(Lexical Environment)** 是一种规范类型，用来定义标识符与基于 ECMAScript 代码嵌套结构的变量和函数之间的联系。词法环境由一个 **环境记录(Environment Records)** 和一个可能为空的到 **外层词法环境的引用(Reference to Outter Lexical Environment)** 构成。通常，词法环境和一些具体的 ECMAScript 代码的语法结构相联系，例如：函数声明，块语句或者是 Try 语句中的 Catch 子语句。这样的代码在每次估值时，新的词法环境就会被创建。

环境记录记录了相应词法环境作用域内创建的标识符绑定。它指向一个词法环境的 EnvironmentRecord。

外部词法环境引用用以建模词法环境值的逻辑嵌套。一个（内部）词法环境的外层词法环境引用是逻辑上包围它的词法环境。一个外层词法环境当然可能有它自己的外层词法环境。一个词法环境可能作为多个内层词法环境的外层词法环境。例如，如果一个函数声明内部包含了 2 个嵌套的函数声明，那么这 2 个嵌套的函数声明逻辑上都以外层函数执行的词法环境作为他们的外层词法环境。

全局环境是一个没有外层环境的词法环境。它的外层环境引用是 null。全局环境的 EnvironmentRecord 可能会被标识符绑定预先填充。全局环境包含一个与它相联系的 Global 对象，这个对象的属性提供一些全局环境的标识符绑定。这个 Global 对象是全局环境的 ThisBinding 的值。随着 ECMAScript 代码的执行，额外的属性可能会被添加到 Global 对象，Global 对象的初始属性也可能会被修改。

模块环境是包含一个模块顶层声明绑定的词法环境，它也包含被显式导入的绑定。一个模块的外层环境是全局环境。

函数环境是一个对应 ECMAScript 函数对象调用的词法环境。一个函数环境可能建立起一个新的 ThisBinding，也可以捕获支持 super 方法调用的必要状态。

词法环境和环境记录值纯粹是规范机制，不需要对应任何 ECMAScript 实现的具体事物。ECMAScript 程序不可能直接访问或者操纵这样的值。

**环境记录**

在这个规范中有 2 种主要的环境记录值被使用：**声明性环境记录(Declarative Environment Records)** 和 **对象环境纪录(Object Environment Records)**。声明性环境记录用于定义 ECMAScript 语言语法元素的效果，例如函数声明，变量声明，以及 Catch 子语句，这些语法元素直接将标识符绑定和 ECMAScript 值联系起来。对象环境记录用于定义 ECMAScript 元素的效果，例如程序和 With 语句将标识符绑定和一些对象的属性联系起来。全局环境记录和函数环境记录是特殊化，他们分别用于脚本全局声明和函数内的顶层声明。

出于规范目的，环境记录值可以被认为存在于一个简单的面向对象的等级结构。在这个结构中，环境记录是一个抽象类，它有 3 个具体子类：声明性环境记录，对象环境记录以及全局环境记录。函数环境记录和模块环境记录是声明性环境记录的子类。

- 声明性环境记录

每个声明性环境记录都和一个包含 var，const，let，class，module，import，function 声明的 ECMAScript 程序作用域相联系。声明性环境记录绑定了由作用域中的声明定义的一系列标识符。

- 对象环境记录

每个对象环境记录都和一个 **绑定对象(Binding Object)** 相联系。对象环境记录绑定了一个字符串标识符名称的集合，这些标识符名称直接对应为绑定对象的属性名。不是字符串形式的标识符名称不被包含进标识符集合中。自己的和继承来的属性都被包含进这个集合中，而不论这些属性的[[Enumerable]]特性设置。因为对象属性可以被动态地增加和删除，绑定在对象环境记录上的标识符集合也可能被潜在地改变，这是任何增加和删除对象属性的操作的副作用。由于这个副作用被创建的任何绑定被视为可变绑定即使对应属性的[[Writable]]特性值为 false。不可变绑定不存在于对象环境记录。

对象环境记录可被配置用于提供他们的绑定对象作为一个隐式的 this 值，这个隐式的 this 值可用于函数调用。这个能力被用来规定导致绑定的 With 语句的行为。这个能力被一个布尔值 provideThis 控制，这个布尔值和每个对象环境记录相联系。默认情况下，任何对象环境记录的 provideThis 值都是 false。

- 函数环境记录

函数环境记录是一个声明性环境记录用来表示一个函数的顶层作用域，并且如果函数不是箭头函数，提供一个 ThisBinding。如果一个函数不是箭头函数并且引用了 super，它的函数环境记录也包含用于从函数内部执行 super 方法调用的状态。

- 全局环境记录

全局环境记录用来表示最外层的作用域，这个作用域被所有在一个一般领域被处理的 ECMAScript 脚本元素共享。一个全局环境记录提供绑定对内置全局对象，全局对象的属性，以及一个脚本内的所有顶层声明。

环境记录对象的基础对象是相关领域的全局对象。这个全局对象是由全局环境记录的 GetThisBinding 具体方法返回的值。全局环境记录的对象环境记录组件包含了所有内置全局对象的绑定，以及由全局代码中的 FunctionDeclaration、GeneratorDeclaration 或 VariableStatement 引入的所有绑定。全局代码中所有其他 ECMAScript 声明的绑定都包含在全局环境记录的声明性环境记录组件中。

一个全局环境记录逻辑上是单个记录，但是它被规定为一个复合对象，它包装了一个对象环境记录和一个声明性环境记录。对象环境记录有他自己的基对象作为关联领域的全局对象。

属性可以直接在一个全局对象上创建。因此，全局环境记录的对象环境记录组件可能包含由 FunctionDeclaration、GeneratorDeclaration 或 VariableDeclaration 声明明确创建的绑定以及作为全局对象的属性隐式创建的绑定。为了识别哪些绑定是使用声明显式创建的，全局环境记录维护一个使用其 CreateGlobalVarBindings 和 CreateGlobalFunctionBindings 具体方法的绑定名称列表。
