...
执行上下文在逻辑上形成一个 **执行上下文栈(Execution Context Stack, ECS)**，又称 **调用栈(Call Stack)**。在这个栈的最上面的执行上下文时运行中的上下文。当控制权从与当前运行的执行上下文相关的可执行代码转移到与该执行上下文无关的可执行代码时，就会创建一个新的执行上下文。新创建的执行上下文被推到堆栈上，成为运行中的执行上下文。当一个 JavaScript 脚本执行时，首先JavaScript 引擎会创建一个全局执行上下文，全局上下文被推入执行上下文栈。当引擎执行流遇到一个函数调用时，便会创建一个对应的函数执行上下文，并把这个函数执行上下文推入执行上下文栈，当这个函数返回时（或者执行完成时，因为函数没有返回语句时，默认返回 undefined），对应的函数执行上下文便会被弹出执行上下文栈。最后当用户关闭网页时，全局上下文便会被弹出。
...
每一种执行上下文都有3个组件记录了执行上下文的状态。**词法环境(LexicalEnvironment)**，**变量环境(VariableEnvironment)**，以及 **this 绑定(ThisBinding)**。
