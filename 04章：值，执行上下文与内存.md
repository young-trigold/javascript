...
当控制权被转移到可执行代码，控制权就进入到一个 **执行上下文(Execution Context)**。活跃的执行上下文逻辑上形成一个栈。这个逻辑栈顶层的执行上下文是 **运行中执行上下文(Running Execution Context)**。每当控制权从与运行中执行上下文联系的可执行代码转移到不与运行中执行上下文联系的可执行代码时，一个新的执行上下文就会被创建。新创建的执行上下文被推入栈，成为运行中执行上下文。
...
每一个执行上下文都包含了 2 个额外的状态组件：LexicalEnvironment 和 VariableEnvironment。

LexicalEnvironment 确定用于解决该执行环境中的代码所做的标识符引用的词法环境。而 VariableEnvironment 识别词法环境，其 EnvironmentRecord 持有由该执行环境中的 var 语句创建的绑定。
...
ES6 规范中有 2 种主要的环境记录值被使用：**声明性环境记录(Declarative Environment Records)** 和 **对象环境纪录(Object Environment Records)**。声明性环境记录用于定义 ECMAScript 语言语法元素的效果，例如函数声明，变量声明，以及 Catch 子语句，这些语法元素直接将标识符绑定和 ECMAScript 值联系起来。对象环境记录用于定义 ECMAScript 元素的效果，例如程序和 With 语句将标识符绑定和一些对象的属性联系起来。全局环境记录和函数环境记录是特殊化，他们分别用于脚本全局声明和函数内的顶层声明。
