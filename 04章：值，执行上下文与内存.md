...
调用 Object 原型上的 toString() 方法，会返回值的一个字符串表示：`'[object Type]'`。其中的 Type 值由 JavaScript 引擎识别。这个方法不但可以识别由构造函数创建出的对象，还能识别一些特殊对象。
...
当控制权被转移到可执行代码，控制权就进入到一个 **执行上下文(Execution Context)**。活跃的执行上下文逻辑上形成一个栈。这个逻辑栈顶层的执行上下文是运行中 **执行上下文(Running Execution Context)**。每当控制权从与运行中执行上下文联系的可执行代码转移到不与运行中执行上下文联系的可执行代码时，一个新的执行上下文就会被创建。新创建的执行上下文被推入栈，成为运行中执行上下文。
