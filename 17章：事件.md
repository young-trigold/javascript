...
btn.addEventListener(
  'click',
  () => console.log('btn received in bubble'),
  false,
);
btn.addEventListener('click', () => console.log('btn received in trap'), true);

document.body.addEventListener(
  'click',
  () => console.log('body received in trap'),
  true,
);

document.body.addEventListener(
  'click',
  () => console.log('body received in bubble'),
  false,
);

document.documentElement.addEventListener(
  'click',
  () => console.log('html received in trap'),
  true,
);

document.documentElement.addEventListener(
  'click',
  () => console.log('html received in bubble'),
  false,
);

document.addEventListener(
  'click',
  () => console.log('document received in trap'),
  true,
);

document.addEventListener(
  'click',
  () => console.log('document received in bubble'),
  false,
);

window.addEventListener(
  'click',
  () => console.log('window received in trap'),
  true,
);

window.addEventListener(
  'click',
  () => console.log('window received in bubble'),
  false,
);

// window received in trap
// html received in trap
// document received in trap
// body received in trap
// btn received in trap
// btn received in bubble
// body received in bubble
// html received in bubble
// document received in bubble
// window received in bubble
```

通过这个例子，我们可以清晰地看到事件到达目标实际上在事件捕获阶段和事件冒泡阶段各有一次，而且事件被传播到了 window。从整体上看，整个事件流是非常对称的。
...
```javascript
const handler = function handler(event) {
  console.log(event.eventPhase, event.currentTarget.tagName);
};
...
导致这个问题的原因主要有两个。第一个是删除带有事件处理程序的元素。比如通过真正的 DOM 方法 removeChild()或 replaceChild()删除节点。最常见的还是使用 innerHTML 整体替换页面的某一部分。这时候，被 innerHTML 删除的元素上如果有事件处理程序，就不会被垃圾收集程序正常清
理。比如下面的例子：
